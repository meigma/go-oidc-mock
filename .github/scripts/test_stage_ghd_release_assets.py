...
class StageGhdReleaseAssetsTest(unittest.TestCase):
    def test_stages_expected_assets(self) -> None:
        with fixture() as root:
            ...
            self.assertEqual(
                ...
                [
                    "checksums.txt",
                    "go-oidc-mock_1.2.3_darwin_amd64",
                    "go-oidc-mock_1.2.3_darwin_amd64.sbom.json",
                    "go-oidc-mock_1.2.3_darwin_arm64",
                    "go-oidc-mock_1.2.3_darwin_arm64.sbom.json",
                    "go-oidc-mock_1.2.3_linux_amd64",
                    "go-oidc-mock_1.2.3_linux_amd64.sbom.json",
                    "go-oidc-mock_1.2.3_linux_arm64",
                    "go-oidc-mock_1.2.3_linux_arm64.sbom.json",
                ],
            )
            linux_binary = root / "dist/release-assets/go-oidc-mock_1.2.3_linux_amd64"
            mode = linux_binary.stat().st_mode
            self.assertTrue(mode & stat.S_IXUSR)
            self.assertIn("dist/release-assets/go-oidc-mock_1.2.3_linux_arm64", stdout)

    def test_fails_on_missing_checksum_entry(self) -> None:
        with fixture(missing_checksum="go-oidc-mock_1.2.3_linux_arm64") as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("missing checksum entry", stderr)
            self.assertIn("go-oidc-mock_1.2.3_linux_arm64", stderr)

    def test_fails_on_checksum_mismatch(self) -> None:
        override = ("go-oidc-mock_1.2.3_linux_amd64", "0" * 64)
        with fixture(checksum_override=override) as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("checksum mismatch for go-oidc-mock_1.2.3_linux_amd64", stderr)
    ...
    def test_fails_on_missing_os_arch_asset(self) -> None:
        with fixture(omit_artifact=("linux", "arm64", "Binary")) as root:
            ...
            self.assertEqual(result, 1)
            self.assertIn("missing expected binary asset go-oidc-mock_1.2.3_linux_arm64", stderr)
...
def run_script(root: Path) -> tuple[int, str, str]:
    stdout = io.StringIO()
    stderr = io.StringIO()
    with working_directory(root), github_repository("meigma/go-oidc-mock"):
...
def fixture(
    *,
    signer: str = "meigma/go-oidc-mock/.github/workflows/release.yml",
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for goos, goarch in PLATFORMS:
            binary_name = f"go-oidc-mock_1.2.3_{goos}_{goarch}"
        ...
        if extra_binary:
            extra_name = "go-oidc-mock_1.2.3_freebsd_amd64"
...
os = "darwin"
arch = "amd64"
pattern = "go-oidc-mock_${{version}}_darwin_amd64"

[[packages.assets]]
os = "darwin"
arch = "arm64"
pattern = "go-oidc-mock_${{version}}_darwin_arm64"

[[packages.assets]]
os = "linux"
arch = "amd64"
pattern = "go-oidc-mock_${{version}}_linux_amd64"

[[packages.assets]]
os = "linux"
arch = "arm64"
pattern = "go-oidc-mock_${{version}}_linux_arm64"
